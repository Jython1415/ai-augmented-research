...
"""Compute Krippendorff's alpha for ICR calibration of relevance coding."""
...
CODER_FILES = [
    "/private/tmp/claude/relevance/icr_results_coder1.json",
    "/private/tmp/claude/relevance/icr_results_coder2.json",
    "/private/tmp/claude/relevance/icr_results_coder3.json",
]

# Load all coder results
coders = []
for path in CODER_FILES:
    with open(path) as f:
        data = json.load(f)
    # Build id -> relevant mapping
    mapping = {}
    for c in data["classifications"]:
        mapping[c["id"]] = 1 if c["relevant"] else 0
    coders.append(mapping)

# Get all post IDs (union across coders)
all_ids = sorted(set().union(*[set(c.keys()) for c in coders]))
print(f"Posts coded: {len(all_ids)}")
print(f"Coders: {len(coders)}")

# Build reliability data matrix (coders x posts)
...
# Compute Krippendorff's alpha (nominal level for binary data)
alpha = krippendorff.alpha(reliability_data, level_of_measurement="nominal")

print(f"\nKrippendorff's alpha: {alpha:.4f}")
if alpha >= 0.800:
    print("Interpretation: STRONG agreement — proceed with confidence")
elif alpha >= 0.667:
    print("Interpretation: ACCEPTABLE agreement — proceed with caution")
else:
    print("Interpretation: BELOW THRESHOLD — revise instructions before proceeding")
...
for j, post_id in enumerate(all_ids):
    ...
    if len(set(values)) == 1:
        ...
        # Find rationales for disagreements
