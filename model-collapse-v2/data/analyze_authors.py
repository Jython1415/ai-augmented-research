...
print("=" * 80)
print("AUTHOR DEMOGRAPHICS AND REACH CORRELATION ANALYSIS")
print("=" * 80)
...
# Get all authors from citation_units joined with author_profiles
cursor.execute("""
    SELECT DISTINCT cu.author_did, ap.bio, ap.followers_count
...
# Analyze top repeat citers
print("\nTop 10 Repeat Citers:")
for i, row in enumerate(repeat_citers[:10], 1):
    author_did = row["author_did"]
    count = row["citation_count"]

    # Get their citations and coding
    cursor.execute("""
        SELECT cu.id, cu.paper_fidelity, cu.field_accuracy, cu.claim_strength, cu.created_at
        FROM citation_units cu
        WHERE cu.author_did = ?
...
# ============================================================================
# 3. REACH CORRELATION: FOLLOWERS VS PAPER FIDELITY & FIELD ACCURACY
# ============================================================================
...
# Map accuracy codes to ordinal values
fidelity_map = {"accurate": 3, "partially_accurate": 2, "misrepresentation": 1, "not_applicable": None}
field_accuracy_map = {"accurate": 4, "partially_accurate": 3, "oversimplified": 2, "inaccurate": 1, "not_applicable": None}
...
# Figure 1: Reach vs Accuracy Scatter Plot
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# Followers vs Paper Fidelity
ax1 = axes[0]
...
ax1.set_title(f"Author Reach vs Paper Fidelity\n(r={corr_fidelity:.3f}, p={p_fidelity:.3f})" if corr_fidelity else "Author Reach vs Paper Fidelity", fontsize=12)
ax1.set_xscale("log")
ax1.grid(True, alpha=0.3)

# Followers vs Field Accuracy
ax2 = axes[1]
...
ax2.set_ylabel("Field Accuracy\n(4=accurate, 3=partial, 2=oversimplified, 1=inaccurate)", fontsize=11)
ax2.set_title(f"Author Reach vs Field Accuracy\n(r={corr_field:.3f}, p={p_field:.3f})" if corr_field else "Author Reach vs Field Accuracy", fontsize=12)
ax2.set_xscale("log")
ax2.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(FIGURES_DIR / "reach_vs_accuracy.png", dpi=300, bbox_inches="tight")
print(f"✓ Saved: {FIGURES_DIR / 'reach_vs_accuracy.png'}")
plt.close()

# Figure 2: Author Followers Distribution
fig, ax = plt.subplots(figsize=(10, 6))

# Get all followers counts
...
print(f"✓ Saved: {FIGURES_DIR / 'author_followers.png'}")
plt.close()
...
cursor.execute("""
    SELECT paper_fidelity, COUNT(*) as count
    FROM citation_units
    WHERE paper_fidelity IS NOT NULL AND paper_fidelity != 'not_applicable'
    GROUP BY paper_fidelity
""")
print(f"\nPaper Fidelity Distribution:")
for row in cursor.fetchall():
    print(f"  {row[0]}: {row[1]}")

cursor.execute("""
    SELECT field_accuracy, COUNT(*) as count
    FROM citation_units
    WHERE field_accuracy IS NOT NULL AND field_accuracy != 'not_applicable'
    GROUP BY field_accuracy
""")
