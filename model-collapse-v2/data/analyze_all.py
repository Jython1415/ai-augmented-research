...
print("=" * 80)
print("PHASE E: COMPREHENSIVE ANALYSIS")
print("=" * 80)
...
# Pass 1 data
cursor.execute("SELECT * FROM citation_units WHERE coding_pass=1")
...
# Pass 2 data
cursor.execute("SELECT * FROM coding_pass2")
...
if pass1_data and pass2_data:
    ...
    pass1_by_id = {row['id']: row for row in pass1_data}
...
cursor.execute("""
    SELECT DISTINCT cu.author_did, ap.followers_count, cu.paper_fidelity, cu.field_accuracy
    FROM citation_units cu
    LEFT JOIN author_profiles ap ON cu.author_did = ap.did
    WHERE cu.coding_pass=1 AND ap.followers_count IS NOT NULL
""")
...
if reach_data:
    ...
    for row in reach_data:
        if row['followers_count'] is not None and row['followers_count'] > 0:
            ...
            fid_map = {'high': 3, 'medium': 2, 'low': 1, None: 0}
...
# Figure 1: Epoch Fidelity Trends
if pass1_data:
    epochs_set = sorted([e for e in set(epochs) if e is not None])
    fidelity_rates = []
    accuracy_rates = []
    ...
    for epoch in epochs_set:
        epoch_fidelities = [paper_fidelities[i] for i, e in enumerate(epochs) if e == epoch]
        epoch_accuracies = [field_accuracies[i] for i, e in enumerate(epochs) if e == epoch]

        # Calculate % of "high" ratings
        fid_high = sum(1 for f in epoch_fidelities if f == 'high') / len(epoch_fidelities) if epoch_fidelities else 0
        acc_high = sum(1 for a in epoch_accuracies if a == 'high') / len(epoch_accuracies) if epoch_accuracies else 0

        fidelity_rates.append(fid_high * 100)
        accuracy_rates.append(acc_high * 100)

    plt.figure(figsize=(10, 6))
    plt.plot(epochs_set, fidelity_rates, marker='o', label='Paper Fidelity (% High)', linewidth=2)
    plt.plot(epochs_set, accuracy_rates, marker='s', label='Field Accuracy (% High)', linewidth=2)
    plt.xlabel('Epoch', fontsize=12)
    plt.ylabel('% High Ratings', fontsize=12)
    plt.title('Accuracy Trends by Epoch', fontsize=14, fontweight='bold')
    plt.legend(fontsize=11)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'epoch_fidelity_trends.png', dpi=150)
    print(f"✓ Saved: epoch_fidelity_trends.png")
    plt.close()

# Figure 2: Two-pass Comparison
if pass1_data and pass2_data and common_ids:
    dimensions = ['claim_strength', 'paper_fidelity', 'field_accuracy']
    pass1_agree = []
    pass2_agree = []
    ...
    for dim in dimensions:
        ...
        p1_high = 100 * sum(1 for v in p1_vals if v == 'high') / len(p1_vals)
        p2_high = 100 * sum(1 for v in p2_vals if v == 'high') / len(p2_vals)

        pass1_agree.append(p1_high)
        pass2_agree.append(p2_high)

    x = np.arange(len(dimensions))
    width = 0.35

    plt.figure(figsize=(10, 6))
    plt.bar(x - width/2, pass1_agree, width, label='Pass 1', alpha=0.8)
    plt.bar(x + width/2, pass2_agree, width, label='Pass 2', alpha=0.8)
    plt.xlabel('Dimension', fontsize=12)
    plt.ylabel('% High Ratings', fontsize=12)
    plt.title('Pass 1 vs Pass 2 Comparison', fontsize=14, fontweight='bold')
    plt.xticks(x, [d.replace('_', ' ').title() for d in dimensions])
    plt.legend(fontsize=11)
    plt.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'twopass_comparison.png', dpi=150)
    print(f"✓ Saved: twopass_comparison.png")
    plt.close()

# Figure 3: Author Followers Distribution
if reach_data:
    ...
    if all_followers:
        plt.figure(figsize=(10, 6))
        plt.hist(all_followers, bins=50, alpha=0.7, color='steelblue', edgecolor='black')
        plt.xlabel('Follower Count', fontsize=12)
        plt.ylabel('Number of Authors', fontsize=12)
        plt.title('Distribution of Author Followers (Log Scale)', fontsize=14, fontweight='bold')
        plt.xscale('log')
        plt.grid(True, alpha=0.3, axis='y')
        plt.tight_layout()
        plt.savefig(FIGURES_DIR / 'author_followers_dist.png', dpi=150)
        print(f"✓ Saved: author_followers_dist.png")
        plt.close()
